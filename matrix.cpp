...
List<Matrix4f> Matrix4f::matrixStack;
...
Vector4f* Matrix4f::Mul(Vector4f* r, Vector4f* a, Vector4f* o, int dimensions) { // r = a * o
    Vector4f t[4];
    Vector4f* d = (r == a) ? t : r;
    if (dimensions < 4)
        memcpy(d, a, sizeof(t));
    for (int col = 0; col < dimensions; col++) {
        ...
        for (int row = 0; row < dimensions; row++) {
            d[row][col] = a[row].Dot(colVec, dimensions);
        }
    }
    if (r == a)
        memcpy(r, t, sizeof(t));
...
Matrix4f Matrix4f::Inverse(void) {
    ...
    if (det == 0.0f) 
        return *this; // oder Fehlerbehandlung
...
#if 1
    m_data[0] = Vector4f{ cosY * cosZ,                              cosY * sinZ,                             -sinY,      0.0f };
    m_data[1] = Vector4f{ sinX * sinY * cosZ - cosX * sinZ,         sinX * sinY * sinZ + cosX * cosZ,       sinX * cosY, 0.0f };
    m_data[2] = Vector4f{ cosX * sinY * cosZ + sinX * sinZ,         cosX * sinY * sinZ - sinX * cosZ,       cosX * cosY, 0.0f };
    m_data[3] = Vector4f{ 0,                                        0,                                       0,          1.0f };
#else
    m_data[0] = Vector4f{ cosY * cosZ, -cosY * sinZ, sinY, 0.0f };
    m_data[1] = Vector4f{ sinX * sinY * cosZ + cosX * sinZ, -sinX * sinY * sinZ + cosX * cosZ, -sinX * cosY, 0.0f };
    m_data[2] = Vector4f{ -cosX * sinY * cosZ + sinX * sinZ,  cosX * sinY * sinZ + sinX * cosZ,  cosX * cosY, 0.0f };
    m_data[3] = Vector4f{ 0, 0, 0, 1 };
#endif
    if (not colMajor)
