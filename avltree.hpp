...
public:
    ...
    public:
        ...
        AvlNode(KEY_T key, DATA_T data) : key(key), data(data), left(nullptr), right(nullptr), balance(0) {}
    };
...
DATA_T* Find(KEY_T key)
{
    if (!m_info.root)
        return nullptr;
    for (tNodePtr node = m_info.root; node != nullptr; ) {
        int rel = m_info.compare(key, node->key);
        if (rel < 0)
            node = node->left;
        else if (rel > 0)
            node = node->right;
...
bool InsertNode(tNodePtr& root)
{
    ...
    int rel = m_info.compare(m_info.key, r->key);
    if (rel < 0) {
